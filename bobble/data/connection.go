...
func GetConnection() {
	...
	databaseUri := fmt.Sprintf("host=%s port=%d user=%s password=pwd%s dbname=%s sslmode=disable", host, port, user, password, dbname)
	...
	connection.AutoMigrate(&model.Sticker{})
